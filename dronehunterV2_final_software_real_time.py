...
from wavelet_transform import WaveletDenoiser
        ...
        for line in self.hackrf.process.stdout:
            if line.strip():
                ...
                if lines_processed >= self.total_bins / FREQ_BINS_PER_LINE:
                    ...
                    power_base = [power_values[i] - self.power_avg[i] for i in range(len(power_values))]
                    detection_results = self.cfar.apply_cfar(power_base)
                    self.spectrum_plot.update(self.frequencies, power_base, self.max_hold)
                    
                    denoised_values = self.denoiser.denoise(np.array(power_base))
                    detection_results = self.cfar.apply_cfar(denoised_values)
                    self.spectrum_plot.update(self.frequencies, denoised_values, self.max_hold)
                    
                    # Overlay detections
                    detected_indices = np.where(detection_results == 1)[0]
                    ...
                    self.waterfall_plot.update(denoised_values)
