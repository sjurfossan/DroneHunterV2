...
class MainWindow(QMainWindow):
    ...
    def read_data(self):
        ...
        detection_result = self.cfar.apply_cfar(powers)

        # Update max hold & spectrum
        self.spectrum_plot.update(freqs, powers, self.max_hold)

        # Overlay detections
        detected_indices = np.where(detection_result == 1)[0]
        detected_freqs = freqs[detected_indices]
        detected_powers = np.array(powers)[detected_indices]
        ...
        if self.count_fn == 0 or self.count_fp == 0:
        ...
        # Waterfall update
        self.waterfall_plot.update(powers)
