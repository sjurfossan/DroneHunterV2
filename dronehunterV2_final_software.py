...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.datasetcontroller = DatasetController("Bymark_data/full_sweep_1.csv")
    ...
    def read_data(self):
        ...
        detection_result = self.cfar.apply_cfar(denoised_values)

        # Update max hold & spectrum
        self.spectrum_plot.update(freqs, denoised_values, self.max_hold)

        # Overlay detections
        detected_indices = np.where(detection_result == 1)[0]
        detected_freqs = freqs[detected_indices]
        detected_powers = np.array(denoised_values)[detected_indices]
        ...
        if self.count_tp == 0 or self.count_fp == 0:
